...
# file paths
SUMMARY_SCRIPT = Path(config_file.SUMMARY_SCRIPT).as_posix()
game_content_summary = imp.load_source("game_content_summary", SUMMARY_SCRIPT)
# save file defaults
...
class GameContentScanner:
    ...
    def start(self) -> None:
        ...
        if asset_registry.is_loading_assets():
            ...
            while asset_registry.is_loading_assets():
                pass
        full_object_paths = unreal.EditorAssetLibrary.list_assets(
            self.ue_folder_path,
            recursive=True,
        )
        self.asset_paths = [GameContentScanner.get_package_name(path) for path in full_object_paths]
    ...
    def finish(self) -> None:
        ...
        # launch next module in order
        unreal.log("Launching content summary module")
        game_content_summary.main(DB_FILE_PATH)
...
if __name__ == "__main__":
    main(UE_FOLDER_PATH_VALUE, DB_FILE_PATH)
